...
class Game(object):
    ...
    def possible_moves(self,player_id:int):
        ...
        col_perimeter = [0,4]
        temp_possible_moves = []
        for row in row_perimeter:
            for col in range(0,5):
                if self.get_board()[col][row] == self.get_current_player() or self.get_board()[col][row] == -1:
                    if col != 0:
                        temp_possible_moves.append(((row, col), Move.TOP))
                    if col != 4:
                        temp_possible_moves.append(((row, col), Move.BOTTOM))
                    if row != 0:
                        temp_possible_moves.append(((row, col), Move.LEFT))
                    if row != 4:
                        temp_possible_moves.append(((row, col), Move.RIGHT))
        for col in col_perimeter:
    ...
    def create_new_state(self, from_pos: tuple[int, int], slide: Move, player_id: int):
        from_pos = (from_pos[1], from_pos[0])
        new_game = deepcopy(self)
        if new_game.__take(from_pos, player_id):
            if not new_game.__slide(from_pos, slide):
                new_game._board[from_pos] = self._board[from_pos]
    ...
    def get_game_state(self) -> str:
        ...
        for row in range(5):
            for col in range(5):
                cell_value = self.get_board()[row, col]
